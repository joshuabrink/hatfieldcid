...
class RouteUtil {
  ...
  deleteEntity = (req, res, next) => {
    ...
    this.collection
      ...
      .then((entity) => {
        ...
        if(this.type == 'groups') {
          let promise = this.updateGroupContacts(entity, false)
  ...
  // setMesageContact = (contactList, numbers) => {
  ...
  //       let promise = Contacts.findContact({ number: number })
  //         .then((c) => {
  //           contact.name = c.name;
  //           return contact;
  //         })
  ...
  updateGroupContacts = (group, queryType)=> {
    return new Promise((resolve, reject) => {
      ...
      for (let i = 0; i < group.contacts.length; i++) {
        let query = { }
        if(queryType) {
          query = {$push: { group: group.name } }
        } else {
          query = {$pull: { group: group.name } }
        }
        
        Contacts.updateEntity(group.contacts[i]._id,query).then(contact=>{
          group.contacts[i] = contact;
          if(i == group.contacts.length-1) {
            Promise.resolve(group).then((g) => {
              resolve(group);
            });
          }
  ...
  addEntity = (req, res, next) => {
    if (this.type == "contacts") {
      ...
      this.collection.addEntity(group).then((g) => {
        let promise = this.updateGroupContacts(group, true);
